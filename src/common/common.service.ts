...
import { TPessoaFisicaOutput, TPessoaJuridicaOutput } from './types/common.types';
...
export class AuxService implements OnModuleInit {
  ...
  async getPjInfo(userId: string): Promise<TPessoaJuridicaOutput> {
    return await this.prismaService.pessoaJuridica.findUnique({ where: { userId: userId }, include: { admins: true } });
  }
